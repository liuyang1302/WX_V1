    ...
    public class Logger
    {
        ...
        public static void WriteTxtLog(string Content)
        {
            try
            {
                ...
                fileName += "WxLog\\";
                fileName += _now.ToString("yyyy-MM-dd-HH") + ".txt";
                string content = Content;
                FileIO fIO = new FileIO();
                fIO.OpenWriteFile(fileName);

                string _nowstr = Environment.NewLine + "****************  " + _now.ToString("yyyy-MM-dd HH:mm:ss") + "  ****************" + Environment.NewLine;
                string _content = _nowstr + content + Environment.NewLine;

                fIO.WriteLine(_content);
                fIO.CloseWriteFile();
            }
            catch
            {
